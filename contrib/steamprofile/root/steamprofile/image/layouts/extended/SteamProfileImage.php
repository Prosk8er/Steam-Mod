...
class SteamProfileImage extends GDImage {
    ...
    private function createPlaceholderImage($iWidth, $iHeight) {
        ...
        $image->copy($errorIcon, $image->getWidth() / 2 - $errorIcon->getWidth() / 2, $image->getHeight() / 2 - $errorIcon->getHeight() / 2);
        $errorIcon->destroy();
    ...
    private function drawStatus($iX, $iY, $sName, $sStatus, $sStatusCode) {
        ...
        // draw text
        $image->drawTextFT($sName, $sFontFileBold, $iFontSizeBold, 0, $iFontColorName, 0, $iFontSizeBold);
        $image->drawTextFT("\n" . $sStatus, $sFontFileNormal, $iFontSizeNormal, 0, $iFontColorStatus, 0, $iFontSizeNormal, array('linespacing' => $fLineSpacing));
    ...
    private function drawGame($iX, $iY, $sGameImageUrl) {
        ...
        // draw game image
        $image->copyResampled($gameImage, 2, 2, 0, 0, $iGameImageDstWidth, $iGameImageDstHeight, $iGameImageSrcWidth, $iGameImageSrcHeight);
        $gameImage->destroy();
    ...
    private function drawInfo($iX, $iY, $sTextLeft, $sTextRight) {
        ...
        // draw text
        $image->drawTextFT($sTextLeft, $sFontFileNormal, $iFontSizeNormal, 0, $iFontColorInfo1, 0, $iFontSizeNormal, array('linespacing' => $fLineSpacing));
        $image->drawTextFT($sTextRight, $sFontFileNormal, $iFontSizeNormal, 0, $iFontColorInfo2, 74, $iFontSizeNormal, array('linespacing' => $fLineSpacing));
    ...
    private function drawIconCluster($iX, $iY, $aIconUrls, $sText) {
        ...
        // draw text
        $image->drawTextFT($sText, $sFontFileNormal, $iFontSizeNormal, 0, $iFontColor, 0, $iFontSizeNormal);
...
//        $this->setSaveFullAlpha(true);
        ...
        $this->copyResampled($errorIcon, 16, $this->getHeight() / 2 - 12, 0, 0, 24, 24, $errorIcon->getWidth(), $errorIcon->getHeight());
        $errorIcon->destroy();
        ...
        // draw text
        $this->drawTextFT($sMessage, $sFontFileNormal, $iFontSizeNormal, 0, $iFontColor, 48, $this->getHeight() / 2, array('linespacing' => $fLineSpacing));
    }
